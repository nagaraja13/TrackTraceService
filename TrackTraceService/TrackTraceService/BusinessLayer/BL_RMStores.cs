...
namespace BusinessLayer
{
    ...
    public class BL_RMStores
    {
        ...
        public DataTable getPickSlipNo(string strUserID)
        {
            SqlDataLayer objSql = new SqlDataLayer();
            try
            {
                return objSql.ExecuteDataset(SqlDataLayer.strLocal, "SELECT DISTINCT (A.PICK_SLIP_NO) AS PICK_SLIP_NO FROM DBO.TBLPICK_SLIP_INWORD A INNER JOIN DBO.TBLPICK_SLIP_ASSIGN B ON A.PICK_SLIP_NO = B.PICK_SLIP_NO AND A.ST in (1, 2) AND B.ST = 0 AND B.PICKER = '" + strUserID.Trim() + "' and (isnull(A.ISSUED_QTY, 0) - Isnull(A.PICKED_QTY, 0)) > 0").Tables[0];
            }
            catch (Exception ex)
            {
                BL_Common.BL_Write(MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name, 0, ex.ToString(), "-");
        ...
        public DataTable getPickSlipData(string strPackSlip, string strUserID)
        {
            SqlDataLayer objSql = new SqlDataLayer();
            try
            {
                return objSql.ExecuteDataset(SqlDataLayer.strLocal, "SELECT A.PICK_SLIP_NO, A.WORK_ORDER_NO, A.ITEM_CODE, REPLACE(REPLACE(A.DESCRIPTION, '@', ''), '$', '') AS DESCRIPTION, A.LOCTAOR , SUM(Isnull(A.ISSUED_QTY, 0)) AS PICK_QTY, SUM(Isnull(A.PICKED_QTY, 0)) AS PICKED_QTY, A.UOM FROM DBO.TBLPICK_SLIP_INWORD A INNER JOIN DBO.TBLPICK_SLIP_ASSIGN B ON A.PICK_SLIP_NO = B.PICK_SLIP_NO AND A.ST in (1, 2) AND B.ST = 0 AND B.PICKER = '" + strUserID.Trim() + "' AND A.PICK_SLIP_NO = '" + strPackSlip.Trim() + "'  and (isnull(A.ISSUED_QTY, 0) - Isnull(A.PICKED_QTY, 0)) > 0 group by A.PICK_SLIP_NO, A.ITEM_CODE, A.WORK_ORDER_NO, A.DESCRIPTION, A.LOCTAOR, A.UOM ORDER BY [dbo].[getAlphaOnly] (dbo.fnsplit(Loctaor, '.', 1)), [dbo].[getNumOnly] (dbo.fnsplit(Loctaor, '.', 1)) ASC").Tables[0];
            }
            catch (Exception ex)
            {
                BL_Common.BL_Write(MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name, 0, ex.ToString(), "-");
